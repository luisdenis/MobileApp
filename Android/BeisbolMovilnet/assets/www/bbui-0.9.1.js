...
//        bb.dropdown.apply(root.querySelectorAll('select'));
		...
		bb.medallero.apply(root.querySelectorAll('[data-bb-type=medallero]'));
		bb.calendar.apply(root.querySelectorAll('[data-bb-type=calendar]'));
		bb.button.apply(root.querySelectorAll('[data-bb-type=button]'));
		bb.menuList.apply(root.querySelectorAll('[data-bb-type=menu-list]'));
		bb.minutoaminuto.apply(root.querySelectorAll('[data-bb-type=minuto-minuto]'));
    ...
    apply: function(elements) {
			...
			for (var i = 0; i < elements.length; i++) {
				...
				var items = outerElement.querySelectorAll('[data-bb-type=item], [data-bb-type=header]'),
					...
					accentText,
					normal,
					highlight,flagNarrative,htmlHeader, typeList = "",
					res;
				...
				for (j = 0; j < items.length; j++) {
					...
					if (innerChildNode.hasAttribute('data-bb-type')) {
						...
						if (innerChildNode.hasAttribute('bb-data-update')) {
							flagNarrative = innerChildNode.getAttribute('bb-data-update');
						}
...
//							innerChildNode.innerHTML = '<p>'+ description +'</p>';
							htmlHeader = '<span >'+ description + '</span>';
							//Game Day
							if(flagNarrative == 1){
								htmlHeader = htmlHeader+'<div class="bb-'+res+'-imagen-list-header-update"><img src="images/update.png" onclick="updateGame(); " /> </div>';
							}
							//Status Innings
							if(flagNarrative == 2){
								var strEventSound = "si";
								if(!flagSoundEvent) strEventSound = "no";
								htmlHeader = htmlHeader+'<div class="bb-'+res+'-imagen-list-header-update"><img src="images/update.png" onclick="updateMenuMinute();" />&nbsp;<img id="idSoundEvent" src="images/sonido_'+strEventSound+'.png" width="26" height="26" onclick="soundEvent();" />  </div>';
							}
							//status Game
							if(flagNarrative == 3){
								var strEventSound = "si";
								...
								var strEvent = "click";
								if(!flagOnlyEvent) strEvent = "normal";
								htmlHeader = htmlHeader+'<div class="bb-'+res+'-imagen-list-header-update" ><img class="onlyEvent-'+res+'" id="idOnlyEvent" src="images/style/commentbutton_'+strEvent+'.png"   onclick="onlyEvent();" />&nbsp;<img class="onlySound-'+res+'"  id="idSoundEvent" src="images/sonido_'+strEventSound+'.png"  onclick="soundEvent();" /> </div>';
							}
							
							innerChildNode.innerHTML = htmlHeader;
...
//							innerChildNode.setAttribute(outEvent, "this.setAttribute('class',this.normal)");
							...
							typeList = "";
							if (innerChildNode.hasAttribute('data-type-list')) {
								typeList = innerChildNode.getAttribute('data-type-list');
							}
							
							if(typeList == "discipline"){
								innerChildNode.setAttribute('class', 'bb-'+res+'-image-list-item-discipline');
								innerChildNode.setAttribute(inEvent, "this.setAttribute('class','bb-"+res+"-image-list-item-hover-discipline')");
								innerChildNode.setAttribute(outEvent, "this.setAttribute('class','bb-"+res+"-image-list-item-discipline')");
								innerChildNode.setAttribute('x-blackberry-focusable','true');
							...
							}else if(typeList == "news" || typeList == "generationGold" ){
								
								innerChildNode.setAttribute('class', 'bb-'+res+'-image-list-item-news');
								innerChildNode.setAttribute(inEvent, "this.setAttribute('class','bb-"+res+"-image-list-item-hover-news')");
								innerChildNode.setAttribute(outEvent, "this.setAttribute('class','bb-"+res+"-image-list-item-news')");
								innerChildNode.setAttribute('x-blackberry-focusable','true');
    ...
    apply: function(elements) {
		// Apply our transforms to all Image Lists
			for (var i = 0; i < elements.length; i++) {
				...
				if (bb.device.isHiRes) {
					outerElement.setAttribute('class','bb-hires-minuto');
				} else {
					outerElement.setAttribute('class','bb-lowres-minuto');
				}
				// Gather our inner items
				...
				for (j = 0; j < items.length; j++) {
					...
					if (innerChildNode.hasAttribute('data-bb-type')) {
						...
						if (innerChildNode.hasAttribute('data-bb-evento')) {
							evento = innerChildNode.getAttribute('data-bb-evento');
							if(evento == ""){ evento = "";}
							else { evento = evento+":&nbsp;";}
						}
						...
						if (innerChildNode.hasAttribute('data-bb-accent-text')) {
							accentText = innerChildNode.getAttribute('data-bb-accent-text');
							accentText = formatFecha(accentText,evento);
						}
						
						// Grab the accent-text if it is there
//						if (innerChildNode.hasAttribute('data-bb-venezuela')) {
//							flagVen = innerChildNode.getAttribute('data-bb-venezuela');
//							
//							if(flagVen == "Venezolanos"){
//								stringBuild = '<img class="title-images-'+res+'"  src="images/flags/VEN.png" />'; 
//							}
//						}
						
						
						
						innerChildNode.setAttribute('class', 'bb-'+res+'-minuto-item');
						innerChildNode.setAttribute(inEvent, "this.setAttribute('class','bb-"+res+"-minuto-item-hover')");
						innerChildNode.setAttribute(outEvent, "this.setAttribute('class','bb-"+res+"-minuto-item')");
						innerChildNode.setAttribute('x-blackberry-focusable','true');
						
						innerChildNode.innerHTML = ''+
										'<div class="details">\n'+
										'   <span class="title" ><b>'+innerChildNode.getAttribute('data-bb-title')+'</b></span>\n'+
										'   <div class="description"> <b>'+evento+'</b> '+ description + '</div>\n'+
										'   <span class="accent-text">' + accentText + '</span>\n'+
										'</div>\n';
						innerChildNode.removeAttribute('data-bb-img');
						innerChildNode.removeAttribute('data-bb-title');
...
bb.menuList = {  
    apply: function(elements) {
		...
		for (var i = 0; i < elements.length; i++) {
			...
			// Gather our inner items
			var items = outerElement.querySelectorAll('[data-bb-type=menu-item]'),
				innerChildNode,
				type,
				j,
				description,
			...
			for (j = 0; j < items.length; j++) {
				...
				if (innerChildNode.hasAttribute('data-bb-type')) {
					...
					normal = 'bb-'+res+'-menu-list-header';
					highlight = 'bb-'+res+'-menu-list-header-hover';
					// Check for alignment
					normal = normal + ' bb-'+res+'-menu-list-header-center';
					highlight = highlight + ' bb-'+res+'-menu-list-header-center';
            ...
            for (var i = 0; i < elements.length; i++) {
                ...
                if (items.length > 0 ) {
                    ...
                    for (var j = 0; j < items.length; j++) {
                        ...
                        // Get the label 
                       var label = row.querySelectorAll('[data-bb-type=label]');
                       for (var k = 0; k < label.length; k++) {
							var col = label[k];
							var tdLabel1 = document.createElement('td');
							tr.appendChild(tdLabel1);
							row.removeChild(col);
							tdLabel1.appendChild(col);
                ...
                if (items.length > 0 ) {
                    ...
                    for (var j = 0; j < items.length; j++) {
                        ...
                        tr = document.createElement('tr');
							// Grab the accent-text if it is there
						flagVen = 0;
						if (row.hasAttribute('data-bb-venezuela')){
							flagVen = row.getAttribute('data-bb-venezuela');
						}
						if(!(flagVen == 1)){
							if(j % 2 == 0)
							tr.setAttribute('class','alt');
